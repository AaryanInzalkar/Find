...
**Verdict: Legitimate pilot candidate.** This is the one place in the
stack where:
- The boundary is clean (bytes in → files out, no shared state)
- Python's `zipfile` is pure Python with no C backend for the
  decompression loop itself
- Gains are proportional to archive size (real user-visible latency)
- The Rust interface is trivially safe: `fn extract(data: &[u8], dest: &Path) -> Vec<String>`

This is the right first Rust contribution if the team wants to introduce
the toolchain.
...
| ZIP archive extraction | Yes (pure Python loop) | Medium–High | Low | Low | **Pilot** |
...
**ZIP extraction**, which is pure Python with a clean FFI boundary and
real user-visible benefit on large uploads.
